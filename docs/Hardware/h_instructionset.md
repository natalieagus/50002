...
* If $$\text{A}_{\text{LE}}$$ is `0`, then R1 will continue to produce it's **old** value as an output, whatever last value it was remembered when $$\text{A}_{\text{LE}}$$ *was* `1` some time in the past. When $$\text{A}_{\text{LE}}$$ is `0`, new values at the **input** node of R1 will be **ignored**. 
